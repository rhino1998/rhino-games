...
func (r *Game) Board(ctx context.Context, _ *Game, category string, game string) (*Board, error) {
	r.lock.Lock()
	defer r.lock.Unlock()
	return r.games[category][game], nil
}

func (r *Game) NewGame(ctx context.Context, _ *Game, category string, game string, x, y int) (bool, error) {
	r.lock.Lock()
	defer r.lock.Unlock()

	board := &Board{}

	r.games[category][game] = board
...
func (r *Game) HitTile(ctx context.Context, _ *Game, category string, game string, x, y int) (TileType, error) {
	r.lock.Lock()
	defer r.lock.Unlock()
	...
	board, ok := gameCategory[game]
	if !ok {
		return 0, fmt.Errorf("Invalid Game %s", game)
	}
